...
// --- MODIFIED MAIN LOOP WITH PROGRESS RESUME ---
const main = async () => {
  logger.banner();
  ...
  while (true) {
    ...
    for (const privateKey of privateKeys) {
      ...
      if (!wprog.checkedIn) {
        ...
        wprog.checkedIn = jwt ? true : false;
        saveProgress(progress);
      } else {
        logger.info('Already checked in for this cycle.');
        // We need JWT for user info
        jwt = null;
      }

      // Record check-in time if success
      if (wprog.checkedIn) {
        thisRunCheckin[wallet.address] = checkinStart;
        // Only fetch user info if just checked in (may skip if already checked in)
        if (jwt) await getUserInfo(wallet, proxy, jwt);
      } else {
