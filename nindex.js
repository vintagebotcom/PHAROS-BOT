...
const numSwaps = 10;
const numLPs = 10;
const WAIT_LOG_INTERVAL = 60 * 10; // Log every 10 minutes while waiting

// Helper: sleep
const sleep = ms => new Promise(res => setTimeout(res, ms));

// Helper: get current UTC time in seconds
const getNowUTC = () => Math.floor(Date.now() / 1000);

// --- MODIFIED MAIN LOOP WITH "CHECK-IN FIRST" LOGIC ---
const main = async () => {
  logger.banner();
  ...
  // Track per-wallet last check-in time (address => unix timestamp)
  let lastCheckinTimes = progress.lastCheckinTimes || {};

  while (true) {
    // Store the check-in time for each wallet in this run
    ...
    for (const privateKey of privateKeys) {
      ...
      // Initialize wallet progress structure if missing
      ...
      // ========== [END CHECK-IN FIRST LOGIC] ==========

      // Optionally fetch user info after checkin
